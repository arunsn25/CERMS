...
com.cosd.greenbuild.calwin.web.library.docdownloadprompt.DownloadPromptContainer" %>
<dmf:html>
<dmf:head>
<dmf:webform/>
...
<h5 style="display:inline;"><dmf:label nlsid='MSG_TITLE'/>:&nbsp;</h5>
</dmf:panel>
...
height='16' tooltipnlsid="MSG_CANCEL_TIP"/> --%>
		...
		<%
		     ...
		     try
		     {
			     String strObjectId = DownloadPromptContainer.downloadPromptObjectID;
			     System.out.println("In container - OBJECTID: " + strObjectId);
			     sHref = request.getContextPath() + "/component/exportwithoutpolicy?objectId=" + strObjectId;
...
<input type="button" value="OK" onclick="parent.window.close();window.open('<%=sHref %>')" style="{font-family:Trebuchet MS, Verdana, GillSans, Arial;color: #333333;font-size: 12px;line-height: 14px;vertical-align: center; font-weight: bold;text-decoration: none}" />
<input type="button" value="Cancel" onclick="parent.window.close()" style="{font-family:Trebuchet MS, Verdana, GillSans, Arial;color: #333333;font-size: 12px;line-height: 14px;vertical-align: center; font-weight: bold;text-decoration: none}" />
